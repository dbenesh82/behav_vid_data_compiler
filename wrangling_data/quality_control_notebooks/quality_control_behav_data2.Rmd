...
There is also no obvious pattern. The duplications occur in different plates, on different days, and in different slices, which is suggestive of human error rather than anything systematic. An obvious question is whether behavior values are the between duplicated slices, suggesting a whole row in the raw data was simply copied. 
...
This was not the case. Activity clearly differed among the multiple measurements on a single slice; compare column `1` vs `2`. This suggests that slice was mislabeled for some reason in the raw data, not that entire rows were copied. I think the best way to remedy these cases, would be to re-evaluate a subsample of the videos to check if the duplicate rows are simply mislabeled.
...
```
We have identified some problems (wrong number of rows, missing observations, duplicate slices) and suggested solutions (deletion, reevaluate videos, accept as missing, etc). In total, `r length(unique(prob_cop_c2))` recordings were considered problematic. This last table summarizes the action taken in each case.
